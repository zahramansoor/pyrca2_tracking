...
addabf = input('Would you like to add abf data? '); %note this adds abf data as "abfdata," to your F files, cut at the imaging points but not aligned or named for generalization purposes 
if addabf
    [abffilename,abfpath] = uigetfile('*.abf','pick your abf file');
    abffullfilename = [abfpath char(abffilename)];
    data = abfload(abffullfilename);  
...
for n = 1:numfiles
    ...
    fullFfile=[Ffilepath{n} Ffile{n}]
     pause(1);
      save(fullFfile,'ybinned','rewards','forwardvel','licks','changeRewLoc','trialnum','timedFF','lickVoltage','VR','-append');
      if addabf
